...
var passwordCheckOld = function () 
{
   ...
   * here
   */
  if(stringPassword.length<8) //checks to see if entered password is at least 8 characters
  {
   alert("Please make sure your password is at least 8 characters long"); //error message
   $("output").value = "";
  }
  else
  {
   for(var i = 0; i < arrayBadPasswords.length; i++)
   {
      ...
      {
         stringOutput = "That is a bad password";
         break;
      }
   }
   if(stringOutput === "That is a bad password")
   {
      $("output").value = stringOutput;
   }
   else
   {
      stringOutput = "That password is ok!";
      $("output").value = stringOutput;
   }
  }
};
